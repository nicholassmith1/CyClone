...
public class TestCCSimpleCC  implements CloneDetectorService {
	...
	static class FakeCloneDetector implements Runnable {
		...
		//https://github.com/takashi-ishio/SimpleCC
		public void run() {
			long start = System.currentTimeMillis();
			ArrayList<String> args = new ArrayList<>();
			args.add("java");
			args.add("-jar");
			args.add("/Users/stevenho/eclipse-workspace/CyClone/lib/simplecc.jar");
			...
			try {
		        ...
		        for(String line:lines) {
		        	if(!line.equals("<pair>") && !line.equals("</pair>")) {
		        		// has char at which clone starts if necessary in output
		        		String[] linecomps = line.split(",");
		        		String filename = linecomps[0];
		        		Integer start_line = Integer.parseInt(linecomps[1]);
		        		Integer end_line = Integer.parseInt(linecomps[3]);
		        		//System.out.println(line);
		        		//System.out.println(filename+" "+start_line+" "+end_line+" "+time_elapsed);
		        		listener.notifyCloneDetected(search, filename,
								start_line, end_line, 99.0,
								"SIMPLECC", time_elapsed);
		        	}
	...
	public static void main(String[] args) {
		...
		FakeCloneDetector detector = new TestCCSimpleCC.FakeCloneDetector(search, listener, 0);
		detector.run();
	}
